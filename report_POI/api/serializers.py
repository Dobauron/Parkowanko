...
class ParkingPointReportSerializer(serializers.ModelSerializer):
    reason = serializers.CharField(validators=[validate_reason])  # Dodajemy walidator do pola
    ...
    def validate(self, attrs):
        ...
        if attrs.get("reason") == ParkingPointReport.ReportReason.OTHER and not attrs.get("description"):
            raise serializers.ValidationError({"error": "Opis jest wymagany dla powodu 'Inne'."})
        return attrs
