...
def user():
    ...
    User = get_user_model()  # Używamy get_user_model do uzyskania niestandardowego modelu użytkownika
...
def test_get_parking_points(api_client, parking_point):
    ...
    assert len(response.data) > 0  # Sprawdzamy, czy w odpowiedzi jest przynajmniej jeden obiekt
...
def test_create_parking_point(api_client, user):
    ...
    assert 'id' in response.data  # Sprawdzamy, czy w odpowiedzi jest identyfikator nowo utworzonego obiektu
...
def test_update_parking_point(api_client, parking_point, user):
    ...
    updated_data = {
        ...
        # "location": {"lat": 52.2298, "lng": 21.0125}
        "location": {"lat": 52.2297, "lng": 21.0122}
    }
    response = api_client.put(f'/api/parkings/{parking_point.id}/', updated_data, format='json')

    assert response.status_code == status.HTTP_200_OK
    assert response.data['name'] == "Updated Parking Point"
    assert response.data['location'] == updated_data['location']
    assert response.data['id'] == parking_point.id  # Sprawdzamy, czy ID pozostało niezmienione
...
def test_update_parking_point_invalid_location(api_client, parking_point, user):
    ...
    response = api_client.put(f'/api/parkings/{parking_point.id}/', invalid_data, format='json')
...
def test_update_parking_point_invalid_location_distance(api_client, parking_point, user):
    """Test sprawdzający walidację lokalizacji, gdzie odległość przekracza dozwoloną wartość"""
    api_client.force_authenticate(user=user)
    invalid_location_data = {
        "name": "Updated Parking Point",
        "location": {"lat": 52.5000, "lng": 21.1000}  # Przykład z odległością większą niż 1000m
    }
    response = api_client.put(f'/api/parkings/{parking_point.id}/', invalid_location_data, format='json')

    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert 'Nowa lokalizacja jest zbyt oddalona od obecnej pozycji: 30641.51m (maksymalnie 1000m).' in str(response.data)
