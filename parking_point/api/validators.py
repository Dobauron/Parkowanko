...
def validate_proximity_to_existing_points(new_lat, new_lng, exclude_id=None):
    ...
    try:
        ...
        raise ValidationError(f"Błąd podczas pobierania istniejących punktów: {str(e)}")
    ...
    for point in existing_points:
        try:
            ...
            raise ValidationError("Błąd w danych lokalizacji istniejących punktów.")

        distance = haversine(new_lat, new_lng, existing_lat, existing_lng)
        if distance < 100:  # Za blisko innego punktu
            raise ValidationError(
                f"Nowa lokalizacja znajduje się zbyt blisko istniejącego punktu: {point.name} ({distance:.2f}m)."
            )
...
def validate_distance_from_current_location(new_lat, new_lng, exclude_id, max_distance):
    ...
    if not isinstance(max_distance, (int, float)) or max_distance < 0:
        raise ValueError("Maksymalna odległość musi być dodatnią liczbą.")

    try:
        current_point = ParkingPoint.objects.get(id=exclude_id)
    except ParkingPoint.DoesNotExist:
        raise ValidationError(f"Punkt parkingowy o ID {exclude_id} nie istnieje.")

    try:
        current_lat = float(current_point.location.get("lat", 0))
        current_lng = float(current_point.location.get("lng", 0))
    except (KeyError, ValueError):
        raise ValidationError("Błąd w danych lokalizacji obecnego punktu.")

    current_distance = haversine(new_lat, new_lng, current_lat, current_lng)
    if current_distance > max_distance:
        raise ValidationError(
            f"Nowa lokalizacja jest zbyt oddalona od obecnej pozycji: {current_distance:.2f}m (maksymalnie {max_distance}m)."
        )
...
def validate_location(new_lat, new_lng, exclude_id=None, max_distance=None):
    ...
    # Debugowanie
    print(
        f"Walidacja lokalizacji: lat={new_lat}, lng={new_lng}, exclude_id={exclude_id}, max_distance={max_distance}"
    )
