...
def user_factory(db, django_user_model):
    def create_user(email=None, username=None):
        ...
        return django_user_model.objects.create_user(email=email, username=username, password="password123")
    return create_user

@pytest.fixture
def parking_point(user_factory):
    user = user_factory()
    return ParkingPoint.objects.create(location={"lat": 52.0, "lng": 21.0}, user=user, has_edit_location_proposal=False)

@pytest.fixture
def proposal(parking_point, user_factory):
    user = user_factory()
    return ParkingPointEditLocation.objects.create(user=user, parking_point=parking_point, location={"lat": 52.0005, "lng": 21.0})
...
def test_set_has_proposal_on_create(parking_point, user_factory):
    ...
    user = user_factory()
    ParkingPointEditLocation.objects.create(user=user, parking_point=parking_point, location={"lat": 52.0001, "lng": 21.0})
...
def test_apply_vote_effect_approve(proposal, user_factory):
    ...
    for u in users:
        ParkingPointEditLocationVote.objects.create(user=u, parking_point_edit_location=proposal, is_like=True)
...
def test_apply_vote_effect_reject(proposal, user_factory):
    ...
    users = [user_factory() for _ in range(3)]
    for u in users:
        ParkingPointEditLocationVote.objects.create(user=u, parking_point_edit_location=proposal, is_like=False)
...
def test_apply_vote_effect_partial_votes(proposal, user_factory):
    ...
    for u in users:
        ParkingPointEditLocationVote.objects.create(user=u, parking_point_edit_location=proposal, is_like=True)
    ParkingPointEditLocationVote.objects.create(user=user_factory(), parking_point_edit_location=proposal, is_like=False)
