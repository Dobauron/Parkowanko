...
def test_vote_serializer_ok(proposal, user_factory):
    ...
    serializer = ParkingPointEditLocationVoteSerializer(
        data=data, context={"proposal": proposal, "request": type("Req", (), {"user": user, "method": "POST"})(), "method": "POST"}
    )
    assert serializer.is_valid(), serializer.errors
...
def test_vote_serializer_already_voted(proposal, user_factory):
    user = user_factory()
    ParkingPointEditLocationVote.objects.create(user=user, parking_point_edit_location=proposal, is_like=True)
    data = {"is_like": False}
    serializer = ParkingPointEditLocationVoteSerializer(
        data=data, context={"proposal": proposal, "request": type("Req", (), {"user": user, "method": "POST"})(), "method": "POST"}
    )
