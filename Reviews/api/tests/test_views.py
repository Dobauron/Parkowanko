...
@pytest.fixture
def user():
    return User.objects.create_user(username="user1", email="user1@test.com", password="pass")

@pytest.fixture
def owner_user():
    return User.objects.create_user(username="owner", email="owner@test.com", password="pass")

@pytest.fixture
def parking_point(owner_user):
    return ParkingPoint.objects.create(location={"lat": 52.0, "lng": 21.0}, user=owner_user)
...
def test_post_review_creates(api_client, parking_point, user):
    ...
    response = api_client.post(f"/api/parking-points/{parking_point.id}/reviews/", data, format="json")
...
def test_post_duplicate_review_returns_error(api_client, parking_point, user):
    api_client.force_authenticate(user=user)
    Review.objects.create(parking_point=parking_point, user=user, occupancy=Review.Occupancy.HIGH, is_like=True)
    data = {"occupancy": Review.Occupancy.LOW, "is_like": False}
    response = api_client.post(f"/api/parking-points/{parking_point.id}/reviews/", data, format="json")
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "Już zgłosiłeś to miejsce" in str(response.data)
