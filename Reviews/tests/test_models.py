...
def test_create_review_ok():
    user = User.objects.create_user(
        email="a@test.com", username="user1", password="pass"
    )
...
def test_unique_together_user_parking_point():
    user = User.objects.create_user(
        email="b@test.com", username="user2", password="pass"
    )
    pp = ParkingPoint.objects.create(location={"lat": 52.0, "lng": 21.0}, user=user)
    ...
    with pytest.raises(IntegrityError):
        Review.objects.create(
            user=user, parking_point=pp, occupancy=Review.Occupancy.LOW, is_like=False
        )
...
def test_attributes_default_empty_list():
    user = User.objects.create_user(
        email="c@test.com", username="user3", password="pass"
    )
    pp = ParkingPoint.objects.create(location={"lat": 52.0, "lng": 21.0}, user=user)

    review = Review.objects.create(
        user=user, parking_point=pp, occupancy=Review.Occupancy.NO_DATA, is_like=True
    )
...
def test_str_method():
    user = User.objects.create_user(
        email="d@test.com", username="user4", password="pass"
    )
    pp = ParkingPoint.objects.create(location={"lat": 52.0, "lng": 21.0}, user=user)
